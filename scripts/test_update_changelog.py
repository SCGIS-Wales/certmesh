...
class TestClassifyPr:
    ...
    def test_refactor_is_changed(self):
        assert uc.classify_pr("refactor: simplify code") == "Changed"

    def test_perf_is_changed(self):
        assert uc.classify_pr("perf: optimize queries") == "Changed"
...
class TestUpdateChangelog:
    ...
    def test_inserts_after_unreleased(self, tmp_path, monkeypatch):
        ...
        assert "## [3.0.1] - 2026-03-14" in result
        assert "New feature" in result
        assert "compare/v3.0.1...HEAD" in result
        assert "[3.0.1]: https://github.com/SCGIS-Wales/certmesh/compare/" in result
        assert "## [3.0.0] - 2026-03-01" in result
        assert "Something old" in result

        unreleased_pos = result.index("[Unreleased]")
        new_pos = result.index("[3.0.1]")
        old_pos = result.index("[3.0.0]")
        assert unreleased_pos < new_pos < old_pos

    def test_skips_existing_version(self, tmp_path, monkeypatch):
        changelog = tmp_path / "CHANGELOG.md"
        changelog.write_text(
            textwrap.dedent("""\
            # Changelog
            ...
            [Unreleased]: https://github.com/SCGIS-Wales/certmesh/compare/v3.0.1...HEAD
            [3.0.1]: https://github.com/SCGIS-Wales/certmesh/releases/tag/v3.0.1
        """)
        )
